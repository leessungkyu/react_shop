...
//useState -> createSlice(리덕트)
let slice1 = createSlice({
  name :'이름',
  initialState:'초기값',
  reducers:{
    //리덕스에 등록된 슬라이스 값을 변경하고자 할면 store에서 변경하는 함수로 등록
    //사용하는 곳에서 그 함수를 사용하는 식으로 변경(간단히 대입x)
    chageData(state, actions){
...
//slice1을 변경하는 함수를 내보낸다(사용하는 곳에서 useDispatch로 사용)
export let {chageData} = slice1.actions;
